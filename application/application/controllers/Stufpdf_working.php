...
class Stufpdf extends CI_Controller
{
	...
	public function index2(){
		...
		echo $this->pdf->stream("sheedpdf.pdf", array("Attachment"=>0));
	...
	public function index(){
		...
		$dob = $this->input->post('birth-month').'/'.$this->input->post('birth-day')."/".$this->input->post('birth-year');
		...
		$schoolSector = $this->input->post('hsgraduated_sector');
		$highestAttained = $this->input->post('year_level');

		// PRESENT ADDRESS
		$presentAddress = $this->input->post('pr_barangay').", ".$this->input->post('pr_towncity').", ".$this->input->post('pr_province');
		$presentZip =  $this->input->post('pr_zip_code');

		// PERMANENT ADDRESS
		$permanentAddress =  $this->input->post('barangay').", ".$this->input->post('towncityname').", ".$this->input->post('province');
		$permanentZip =  $this->input->post('zipcode');

		$schoolLastAttended = $this->input->post('hsgraduated');
		$schoolLastAttendedAddress = $this->input->post('hsgraduated_address');
		
		$disability = $this->input->post('disability');
		$ipAffiliation = $this->input->post('ip_affiliation');

		// FATHER
		$fatherStatus =  $this->input->post('fstatus');
		$fatherName =  $this->input->post('father')." ".$this->input->post('father_m')." ".$this->input->post('father_l');
		$fatherAddress =  $this->input->post('f_address');
		$fatherContactNo =  $this->input->post('f_contactno');
		$fatherOccupation =  $this->input->post('foccupation');
		$fatherEmployer =  $this->input->post('f_employer');
		$fatherEmployerAddress =  $this->input->post('f_employer_address');
		$fatherAttained =  $this->input->post('f_highesteduc');
		$fatherIncome =  $this->input->post('income');

		// MOTHER
		$motherStatus =  $this->input->post('mstatus');
		$motherName =  $this->input->post('mother')." ".$this->input->post('mother_m')." ".$this->input->post('mother_l');
		$motherAddress =  $this->input->post('m_address');
		$motherContactNo =  $this->input->post('m_contactno');
		$motherOccupation =  $this->input->post('moccupation');
		$motherEmployer =  $this->input->post('m_employer');
		$motherEmployerAddress =  $this->input->post('m_employer_address');
		$motherAttained =  $this->input->post('m_highesteduc');
		$motherIncome =  "";

		// GUARDIAN
		$guardianStatus = '';
		$guardianName = $this->input->post('guard_fname')." ".$this->input->post('guard_mname')." ".$this->input->post('guard_lname');
		$guardianAddress = $this->input->post('guard_address');
		$guardianContactNo = $this->input->post('guard_contact');
		$guardianOccupation = $this->input->post('guard_occupation');
		$guardianEmployer = $this->input->post('guard_employer');
		$guardianEmployerAddress = $this->input->post('guard_empaddress');
		$guardianAttained = $this->input->post('guard_highesteduc');
		$guardianIncome = '';

		$noOfSiblings =  $this->input->post('siblingno');

		$isDSWDBen =  $this->input->post('dswd4p');

		$schoolIntendedToEnroll =  $this->input->post('heiname');
		$schoolIntendedToAddress =  $this->input->post('hei_address');
		$schoolIntendedType = $this->input->post('hei_type');

		$degree = $this->input->post('course');
		$degreePriority = '';
		


		$enjoyingOtherSources = $this->input->post('other_assistance');

		$firSourceType = $this->input->post('othertype1');
		$firSourceAgency = $this->input->post('othertype1_name');
		$secSourceType = $this->input->post('othertype2');
		$secSourceAgency = $this->input->post('othertype2_name');

		$printedName = $lname.', '.$fname.' '.$mname;
...
// END CHEDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
		...
		$fatherAddressFont = '';
		if($fatherAddressLen > 37) $fatherAddressFont = '; font-size: 5pt';
		...
		$motherAddressFont = '';
		if($motherAddressLen > 48) $motherAddressFont = '; font-size: 5pt';
		...
		$motherEmployerFont = '';
		if($motherEmployerLen > 48) $motherEmployerFont = '; font-size: 5pt';
		...
		$html_content .= '
		<div>
			<b><i><font style="font-size: 7.5pt;">CHED-Scholarship Application Form<br>2019 version </font><font style="30pt;">(NOT FOR SALE)</font></i></b>
			<div style="margin-left: 677px; margin-top: -29px; font-family: Arial, sans-serif; font-size: 11pt;"><b><font>ANNEX "A"</font></div></div><br>
			<div class="colored" style="float: right; margin-top: -25px; width: 118px; height: 118px; border: 2px solid black; margin-right: 3px;">
				<center style="font-size: 7.5pt; margin-top: 48px;">2 X 2<br>
				ID PICTURE</center>
			</div>

			<div style="margin-top: 20px; margin-left: 40px; width: 132px; height: 132px; margin-right: 3px;"><img src="public/img/ched-logo.png" height="70px;"></div>
			<div style="margin-top: 10px; margin-left: 40px; margin-right: 3px; font-family: Arial, sans-serif; font-size: 7.5pt; margin-top: -113px;"><center><b>Office of the President of the Philippines<br>COMMISSION ON HIGHER EDUCATION<br>REGIONAL OFFICE 1</b></center></div>
			<div style="margin-top: 10px; margin-left: 30px; margin-right: 3px; font-family: Century Gothic, sans-serif; font-size: 11pt;"><center><b>CHED STUDENT FINANCIAL ASSISTANCE PROGRAMS (StuFAPs)<br><font style="font-size: 12pt;">APPLICATION FORM</font></b></center></div>
		</div>
        ...
        <table class="b" style="border-collapse: collapse;border-left: none; border-right: none;">
        	<tbody hidden>
        		<tr>
        			<td colspan="13">
	    				<div>
							<div style="margin-left: 2px; margin-right: 3px; font-family: Arial, sans-serif; font-size: 9pt; font-weight: bold;">
								Application period: March 1 to May 31 of the current academic year
							</div>
						</div>
					</td>
        		</tr>
        ...
        $html_content .= '
        				...
        				<tr>
        					...
        					<td colspan="5" style="border-right: 1px solid black; overflow:auto; table-layout: fixed" class="input"><center>'.$lname.'</center></td>
        				...
        				<tr>
        					...
        					<td colspan="4" rowspan="2" style="border-top: 2px solid black;" width="40%" class="input">'.$presentAddress.'</td>
        				</tr>
        				<tr>
        				...
        				<tr>
        					...
        					<td colspan="4" valign="middle" style="border-right: 1px solid black; border-top: 1px solid black; border-bottom: 1px solid black">
        						<div style="height: 12px;">
        							<input type="checkbox" style="margin-top: -8.8pt;" '.$sexMale.'>
        							<div style="margin-top: -17px; margin-left: 17px;">Male</div>
        							<font style="margin-bottom: 10pt;">
        							<input type="checkbox" style="margin-top: -10.5px; margin-left: 50pt" '.$sexFemale.'></font>
        							<div style="margin-top: -30px; margin-left: 83px;">Female</div>
        				...
        				<tr>
        					...
        					<td colspan="4" rowspan="3" valign="middle" style="border-right: 2px solid black; border-bottom: 1px solid black" height="30">
	        					<div style="height: 12px;  margin-top: 1px;">
        							<input type="checkbox" style="margin-top: -8.8pt;" '.$civilStatusSingle.'>
        							<div style="margin-top: -17px; margin-left: 17px;">Single</div>
	        					...
	        					<div style="height: 12px; margin-top: 15px;">
        							<input type="checkbox" style="margin-top: -8.8pt;" '.$civilStatusMarried.'>
        							<div style="margin-top: -17px; margin-left: 17px;">Married</div>
	        					...
	        					<div style="height: 12px; margin-top: 15px;">
        							<input type="checkbox" style="margin-top: -8.8pt;" '.$civilStatusAnnulled.'>
        							<div style="margin-top: -17px; margin-left: 17px;">Annulled</div>
        							<font style="margin-bottom: 10pt;"><input type="checkbox" style="margin-top: -10.5px; margin-left: 50pt" '.$civilStatusOthers.'></font>
        							<div style="margin-top: -29px; margin-left: 83px;">Others</div>
	        					</div>
        					
        					</td>
        					<td colspan="3" rowspan="2" class="colored" style="border-right: 1px solid black">9. Permanent Address</td>
        					<td colspan="4" class="input" rowspan="2" style="border-bottom: 1px solid black">'.$permanentAddress.'</td>
        				</tr>
        				<tr>
        				...
        				<tr>
        					...
        					<td colspan="2" style="border-bottom: 1px solid black '.$guardianAddressFont.'" class="input">'.$guardianAddress.'</td>
        				</tr>
        				<tr>
        				...
        				<tr>
        					...
        					<td colspan="2" style="border-bottom: 1px solid black '.$guardianEmployerAddressFont.'" class="input">'.$guardianEmployer.'</td>
        				</tr>
        				<tr>
        				...
        				<tr>
        					...
        					<td colspan="6" style="border-bottom: 1px solid black">'.$degree.'</td>
        					<td colspan="2"></td>
        				</tr>
        				<tr>
	        			...
	        			<tr height="6">
	        				<td colspan="9"><center class="input" style="margin-top: 13px;">'.$printedName.'</center></td>
        					<td colspan="4">
        				...
        				<tr>
        					...
        					<td colspan="6"><i><b style="margin-left: 20px;">(<b>'.$reportCard.'</b>) Report Card (<b>'.$grades.'</b>) Copy of Grades: Grade 11 or 1st semester of Grade 12</i></b></td>
        				</tr>
        				<tr>
        					<td colspan="7" style="border-right: 1px solid black;">
        						<div style="height: 12px;">
        				...
        				<tr>
        					...
        					<td colspan="10">(<b>'.$schoolSectorPublic.'</b>) Public (<b>'.$schoolSectorPrivate.'</b>) Private</td>
        				</tr>
        				<tr>
        					<td colspan="3" style="border-right: 1px solid black">Course</td>
        					<td colspan="5" class="input">'.$degree.'</td>
        				...
        				<tr>
	        				<td colspan="7" style="border-top: 2px solid black; border-bottom: 2px solid black; border-right: 2px solid black;" ><b>CRITERIA OF ELIGIBILITY per  CMO______ s. 2019</b>
	        					<ol style="margin-top: auto; margin-left: -20px;">
	        						...
	        						<b style="font-size: 6pt">NOTE:<br>Beneficiaries of free higher education under RA 10931 can only receive stipend under this program</b>
	        					</ol>
	        					

	        				</td>
	        				<td colspan="6" style="border-top: 2px solid black;border-bottom: 2px solid black;" width="75%"><b>DOCUMENTARY REQUIREMENTS  per  CMO______ s. 2019</b>
	        					<br>Academic Requirements - any one of the following:
	        					<br>( &nbsp;&nbsp;) &nbsp;&nbsp;1. High school report card
	        					<br>( &nbsp;&nbsp;) &nbsp;&nbsp;2. Duly certified true copy of grades for Grade 11 or 1st semester of Grade 12 for<br><u><font style="margin-left: 24px;">graduating high school students</font></u>
	        					...
	        					<br>( &nbsp;&nbsp;) &nbsp;&nbsp;4.Â Certificate/Case Study from DSWD
