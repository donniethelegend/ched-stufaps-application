...
class Eteeap_model extends CI_Model
{
	...
	public function get()
	{
		...
		$permits = $this->db->query("SELECT * FROM hei_eteeap LEFT JOIN a_institution_profile ON hei_eteeap.instcode = a_institution_profile.instcode");
	...
	public function saveeteeap($instcode,$programname,$contact,$eteeap_status,$remarks)
	{
		
		$sql = "INSERT INTO hei_eteeap (instcode,programname,contact,eteeap_status,remarks) VALUES (".$this->db->escape($instcode).", ".$this->db->escape($programname).", ".$this->db->escape($contact).", ".$this->db->escape($eteeap_status).", ".$this->db->escape($remarks).")";
		$this->db->query($sql);
	...
	{
		$this->db->delete('hei_eteeap', array('eteeapid' => $eteeapid));
