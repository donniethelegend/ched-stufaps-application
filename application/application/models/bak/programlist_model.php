...
class Programlist_model extends CI_Model
{
	...
	public function get($search_instcode)
	{
		...
		$result = $this->db->query("SELECT * FROM b_program LEFT JOIN a_institution_profile ON b_program.instcode = a_institution_profile.instcode left join ref_b_discipline_group ON b_program.discipline = ref_b_discipline_group.groupcode where b_program.instcode=".$this->db->escape($search_instcode)." AND program_deleted=0");
...
WHERE b_program_graduate.g_programid= ".$this->db->escape($search_instcode_post)." AND graduate_deleted=0");
	...
	{
		$query = $this->db->query("SELECT * from ref_b_discipline_group");
		return $query->result_array();

	}
	public function getdiscipline_specific()
	{
		$query = $this->db->query("SELECT * from ref_b_discipline_specific");
		return $query->result_array();
	...
	public function programsearch($programnamesearch)
	{
		...
		$query = $this->db->query("SELECT * FROM b_program LEFT JOIN a_institution_profile ON b_program.instcode = a_institution_profile.instcode WHERE mainprogram LIKE ".$this->db->escape($clean_search)." AND program_deleted=0");
		return $query->result_array();
