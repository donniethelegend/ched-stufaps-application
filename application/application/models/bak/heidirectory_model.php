...
class Heidirectory_model extends CI_Model
{
	...
	public function getprograms($icode)
	{
			...
			$this->db->where('instcode',$icode);
			$progs = $this->db->get(); */
			$progs = $this->db->query("SELECT programlevel,programid, mainprogram,supervisor,major,pstatuscode,authcat,authserial,authyear,discipline,discipline2,remarks FROM b_program where instcode=".$this->db->escape($icode)." AND program_deleted=0");
	...
	public function getxls()
	{
		...
		$heidir = $this->db->query("SELECT instname, b_program.instcode,supervisor,datayear,programlevel,mainprogram,mpcode,major,mjcode,discipline,discipline2,thesisdisert,pstatuscode,pstatyear,authcat,authserial,authyear,remarks,pmcode,pmif,nlyears,b_program.pcredit,b_program.tuitionperunit,b_program.programfee,newstudm,newstudf,oldstudm,oldstudf,secondm,secondf,thirdm,thirdf,fourthm,fourthf,fifthm,fifthf,sixthm,sixthf,seventhm,seventhf,gradm,gradf,(newstudm+newstudf+oldstudm+oldstudf+secondm+secondf+thirdm+thirdf+fourthm+fourthf+fifthm+fifthf+sixthm+sixthf+seventhm+seventhf) AS totalenrol, (gradm+gradf) AS totalgrad FROM b_program LEFT JOIN b_program_enrolment ON b_program.programid = b_program_enrolment.e_programid LEFT JOIN b_program_graduate ON b_program.programid = b_program_graduate.g_programid LEFT JOIN a_institution_profile ON b_program.instcode = a_institution_profile.instcode WHERE program_deleted=0");
	...
	{
		//$result = $this->db->get('contacts');
		$heidir = $this->db->query("SELECT * FROM a_institution_profile where hei_status='ACTIVE'");
		return $heidir->result_array();
	...
	public function getfaculty($icode)
	{
			...
			$department = $this->db->query("SELECT facultyid, faculty_name,fullpart_description,faculty_gender,highestdegree_description,faculty_programname,faculty_masters,faculty_doctorate,license_description,tenure,rank_description,teaching_description, subjects, salary_description,faculty_status
FROM a_faculty
...
LEFT JOIN ref_f_salary ON a_faculty.annual_salary_code = ref_f_salary.salarycode where instcode=".$this->db->escape($icode)."");
	...
	public function saveheiinfo($heitype,$province2,$insthead,$titlehead,$atel,$afax,$ainsttelhead,$aemail,$awebsite,$instname,$form_ownership,$astreet,$amunicipality,$aprovince,$aregion,$azip,$aestablished,$asec,$ayearapproved,$acollege,$auniversity,$instcode,$insttype,$insttype2,$hei_remarks,$hei_status,$latitude,$longtitude,$heid,$highesteduced)
	{
		...
		$heiprofile = $this->db->query("SELECT * FROM a_institution_profile where instcode=".$this->db->escape($instcode)."");
		$heiprofile_info = $heiprofile->result_array();
		print_r("SELECT * FROM a_institution_profile where instcode=".$this->db->escape($instcode)."");
		print_r($heiprofile_info);
		//remarks
		...
		if($heiprofile_info[0]['longtitude']!=$longtitude){
			$remarks.="Longitude from ".$heiprofile_info[0]['longtitude']. " to " .$this->db->escape($longtitude)."\n";
		}
		if($heiprofile_info[0]['hei_status']!=$hei_status){
			$remarks.="Status from ".$heiprofile_info[0]['hei_status']. " to " .$this->db->escape($hei_status)."\n";
		}
		if($heiprofile_info[0]['highesteduchead']!=$highesteduced){
			$remarks.="Head Educational Attainment from ".$heiprofile_info[0]['highesteduchead']. " to " .$this->db->escape($highesteduced)."\n";
		}
		

		if($remarks!=""){
			$sql_log = "INSERT INTO a_institution_profile_e_log (aprofilelogeremarks,regionuser) VALUES (".$this->db->escape($remarks).", ".$this->db->escape($username).")";
			$this->db->query($sql_log);
		}
		
		
		
		$sql = "update a_institution_profile set insttel=".$this->db->escape($atel).", instfax=".$this->db->escape($afax).", insttelhead=".$this->db->escape($ainsttelhead).", email=".$this->db->escape($aemail).", website=".$this->db->escape($awebsite).", insthead=".$this->db->escape($insthead).", titlehead=".$this->db->escape($titlehead).", heitype=".$this->db->escape($heitype).", province2=".$this->db->escape($province2).", instname=".$this->db->escape($instname).", instformownership=".$this->db->escape($form_ownership).", street=".$this->db->escape($astreet).", municipality=".$this->db->escape($amunicipality).", province1=".$this->db->escape($aprovince).", region=".$this->db->escape($aregion).", postalcode=".$this->db->escape($azip).", established=".$this->db->escape($aestablished).", sec=".$this->db->escape($asec).", yearapproved=".$this->db->escape($ayearapproved).", tocollege=".$this->db->escape($acollege).", touniversity=".$this->db->escape($auniversity).", insttype=".$this->db->escape($insttype).", insttype2=".$this->db->escape($insttype2).", hei_remarks=".$this->db->escape($hei_remarks).", hei_status=".$this->db->escape($hei_status).", latitude=".$this->db->escape($latitude).", longtitude=".$this->db->escape($longtitude).", highesteduchead=".$this->db->escape($highesteduced)." where instcode=".$this->db->escape($instcode)."";
	...
	public function deleteformername($formerid)
	{
		//$this->db->delete('contacts', array('programid' => $programid));
		$sql = "update a_formernames set formername_deleted='1' where formerid=".$this->db->escape($formerid)."";

		$this->db->query($sql);
		
	}
	
	public function updateinstcode($instcode,$heid)
	{
		//$this->db->delete('contacts', array('programid' => $programid));
		$sql = "update a_institution_profile set instcode=".$this->db->escape($instcode)." where heid=".$this->db->escape($heid)."";
		...
		$this->session;	
		$username = $this->session->userdata('username');
		$remarks = "Institutional Code updated to ".$instcode;
		$sql_log = "INSERT INTO a_institution_profile_e_log (aprofilelogeremarks,regionuser) VALUES (".$this->db->escape($remarks).", ".$this->db->escape($username).")";
		$this->db->query($sql_log);
	}
}
